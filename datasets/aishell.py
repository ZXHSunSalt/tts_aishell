...
def build_from_path(in_dir, out_dir, num_workers=1, tqdm=lambda x: x):
    ...
    futures = []
    index = 1
    with open(os.path.join(in_dir, 'aishell.csv')) as f:
...
#             print(wav_path)
            text = parts[2]
            test = partial(_process_utterance, out_dir, index, wav_path, text)
...
def _process_utterance(out_dir, index, wav_path, text):
  ...
  spectrogram_filename = 'aishell-spec-%05d.npy' % index
  mel_filename = 'aishell-mel-%05d.npy' % index

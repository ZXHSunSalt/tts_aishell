...
from tacotron.datasets import blizzard, ljspeech, aishell
...
def preprocess_aishell(args):
    in_dir = os.path.join(args.base_dir, 'AishellSpeech')
    out_dir = os.path.join(args.base_dir, args.output)
    os.makedirs(out_dir, exist_ok=True)
    metadata = aishell.build_from_path(in_dir, out_dir, args.num_workers, tqdm=tqdm)
    write_metadata(metadata, out_dir)
...
def main():
    ...
    parser.add_argument('--base_dir', default=os.path.expanduser('E:\\DATA\\tts'))
    parser.add_argument('--output', default='training')
    parser.add_argument('--dataset', required=True, choices=['blizzard', 'ljspeech', 'aishell'])
